<!DOCTYPE html>
<html lang="pl">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>BetterFlights</title>
    <link rel="stylesheet" href="css/styleMain.css">
</head>
<body>
<!-- Nagłówek -->
<header>
    <div class="container">
        <div class="logo">
            <h1>BetterFlights</h1>
        </div>
        <nav>
            <ul>
                <li><a href="strony/logowanie.php">Panel Logowania</a></li>
                <li><a href="rezerwacje/formularz_wyszukiwania.php">Panel Wyszukiwania</a></li>
                <li><a href="strony/panel_uzytkownika.php">Panel Użytkownika</a></li>
                <li><a href="strony/panel_administratora.php">Panel Administratora</a></li>
            </ul>
        </nav>
    </div>
</header>

<!-- Sekcja główna -->
<div class="hero">
    <h1>BetterFlights</h1>
    <p>Znajdź najlepsze loty na świecie. Komfort, bezpieczeństwo i wygoda w jednym miejscu.</p>
    <a href="rezerwacje/formularz_wyszukiwania.php" class="btn">Sprawdź loty</a>
</div>

<!-- Stopka -->
<footer>
    <p>&copy; 2025 BetterFlights. Wszystkie prawa zastrzeżone.</p>
</footer>

<script src="js/skrypt.js"></script>
</body>
</html>
